...
class FlightsList extends Component {
  ...
  onSubmit = (data) => {
    axios.put(`http://localhost:8000/flights/${data._id}`, data).then(() => {
      this.setState((prev) => ({
        flights: prev.flights.map((row) => (row._id === data._id ? data : row)),
        permanentFlights: prev.permanentFlights.map((row) =>
          row._id === data._id ? data : row
        ),
        openEditDialog: false,
        dialogFlight: null,
      }));
  ...
  filterFlight = () => {
    ...
    if (
      ...
      depTime === null &&
      arrTime === null &&
      depTerminal.length === 0 &&
      arrTerminal.length === 0 &&
      cabin.length === 0 &&
      seats.length === 0
    ) {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        headerName: "From",
        width: 140,
        renderCell: (params) => <div>{params.value.airport}</div>,
      },
      {
    ...
    return (
      <div>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="alert-dialog-description">
              Pressing Yes will delete the Flight with ID{" "}
              {this.state.dialogFlight}.
            </DialogContentText>
          </DialogContent>
          <DialogActions>
            ...
            <Button
              onClick={this.onDialogCloseDelete}
              color="primary"
              autoFocus
            >
              Yes
        ...
        <Snackbar
          ...
          onClose={this.deleteSnackBarHandleClose}
        >
          <Alert
            onClose={this.deleteSnackBarHandleClose}
            severity="success"
